...
# Dados de exemplo com coordenadas (latitude, longitude)
dados = pd.DataFrame({
    "Nome": ["Rancho",
             "Chácara charmosa",
             "Chácara agradável",
...
# Exibindo dados no Streamlit
st.markdown("### Chácaras Filtradas")
st.dataframe(
    dados_filtrados.sort_values(
        by="Avaliação", ascending=False, ignore_index=True
    )
)
...
# Defina as coordenadas de partida (por exemplo, sua casa)
start_coords = [-22.849131, -47.113239]  # Exemplo de coordenadas da sua casa
...
for i in rotas_selecionadas:
    end_coords = [dados.iloc[i]["Latitude"], dados.iloc[i]["Longitude"]]  # Coordenadas da chácara
    route_geometry, distance, duration = get_route(start_coords, end_coords)  # Calcula a rota
